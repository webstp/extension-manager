...
export function activate(context: vscode.ExtensionContext) {
    ...
    let installMissingExtensions = vscode.commands.registerCommand('extension.installMissing', () => {
            ...
            if (extensions.length === 0) {
                ...
                listExtensions(extensions).then(extensions => {
                    ...
                    Promise.all(promises).then(() => {
                        vscode.window.showInformationMessage(`Finished installing ${finished} extensions`);
                    });
                });
            }
        });
